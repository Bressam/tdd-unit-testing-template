...
class ListViewController: UITableViewController {
    ...
    // moved to global variables, so the class is more testable
    // ideally these services should be passed by dependency injection on init
    var friendsService: FriendsAPI = FriendsAPI.shared
	...
	@objc private func refresh() {
		...
		if fromFriendsScreen {
            self.friendsService.loadFriends { [weak self] result in
	...
	private func handleAPIResult<T>(_ result: Result<[T], Error>) {
		...
		case let .failure(error):
            ...
            if fromFriendsScreen && self.user?.isPremium == true {
